# Perception of Engineering Students on Fruit Preference
...
df=read.csv("https://raw.githubusercontent.com/sijuswamy/Data-Analytics-using-R/main/Survey_Data.csv.csv",header = TRUE)
df=df[,-1]
size=nrow(df)
#df
```
...
This article is prepared during the hands-on sessions of the first phase of the Faculty Development Programme- `Data Analytics Using R`. A questionnaire is prepared and administered through *Google form*. Total number of samples collected in the study is `r size`.
